document.addEventListener("DOMContentLoaded", () => {
  window.location.href = "/termsPage.html#terms"
...
function checkSignUpForm() {
  ...
  if (signEmailInput.value === "" || !emailPattern.test(signEmailInput.value)) {
    signEmailError.style.display = "block";
    valid = false;
  } else {
    signEmailError.style.display = "none";
  }
  if (signPassInput.value === "" || signPassInput.value.length < 8) {
    signPassError.style.display = "block";
    valid = false;
  } else {
    signPassError.style.display = "none";
  }
  if (
    signConfPassInput.value === "" ||
    !(signPassInput.value === signConfPassInput.value)
  ) {
    signConfirmPassError.style.display = "block";
    valid = false;
  } else {
    signConfirmPassError.style.display = "none";
  }

  // Save User Info and Proceed if valid
  if (valid) {
    localStorage.setItem("userEmail", signEmailInput.value);
    localStorage.setItem("userPassword", signPassInput.value);
    isLoggedIn = true;
    localStorage.setItem("isLoggedIn", "true");
    updateAuthButton();
...
authButton.addEventListener("click", () => {
  if (isLoggedIn) {
    ...
    // Handle Login/Sign-Up Prompt
    const isSignUp = confirm(
      "Do you want to sign up? Press 'Cancel' for Login."
    );
    if (isSignUp) {
      checkSignUpForm();
    } else {
