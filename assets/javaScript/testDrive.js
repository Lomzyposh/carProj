...
document.addEventListener("click", (event) => {
  if (event.target.id === "logBut") {
    handleLogin();
    bootstrap.Modal.getInstance(document.getElementById("log2")).hide();
...
// Populate the brand dropdown with unique brands
...
brandDropdown.addEventListener("change", function () {
  ...
  // Clear and enable model dropdown
  modelDropdown.innerHTML =
    '<option value="" selected disabled>Select a Model</option>';
  modelDropdown.disabled = false;

  // Filter car data to get models for the selected brand
  const models = cars
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const toastLiveExample = document.getElementById("liveToast");
  const brandCheck = document.getElementById("brand");
  const modelCheck = document.getElementById("model");
  ...
  Array.from(forms).forEach((form) => {
    form.addEventListener(
      ...
      (event) => {
        ...
        const emailField = form.querySelector("#validationEmail");
        const zipCode = form.querySelector("#validationCustom05");
        const city = form.querySelector("#validationCustom04");
        ...
        if (!emailRegex.test(emailField.value)) {
          ...
          isValid = false;
          emailFeed.innerHTML = "Enter valid Email";
          emailFeed.classList.add("text-danger");
        } else {
          emailField.classList.remove("is-invalid");
          emailFeed.innerHTML = "Looks good!";
          emailFeed.classList.remove("text-danger");
        }
        if (zipCode.value === "") {
  ...
  // Button Disable/Enable Logic Based on Input Fields
  function disBut() {
    const brandValue = brandCheck.value.trim();
    const modelValue = modelCheck.value.trim();

    if (!brandValue || !modelValue) {
      subBut.style.cursor = "not-allowed";
      subBut.disabled = true;

      carError.style.display = "block"; // Show error message
      subBut.classList.remove("btn-primary");
      subBut.classList.add("btn-outline-secondary");
    } else {
      subBut.disabled = false;
      subBut.style.cursor = "pointer";
      carError.style.display = "none"; // Hide error message
      subBut.classList.add("btn-primary");
      subBut.classList.remove("btn-outline-secondary");
    }
  }

  // Initial State and Event Listeners
  brandCheck.addEventListener("change", disBut);
  modelCheck.addEventListener("change", disBut);
  disBut(); // Set initial state
});
